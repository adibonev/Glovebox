...
def draw_wheel(img: Image.Image, rim_r: float) -> None:
    ...
    # rim
    d.ellipse([cx - rim_r, cy - rim_r, cx + rim_r, cy + rim_r], outline=copper, width=round(stroke))
    # spokes: full horizontal (9+3 o'clock) and a single downward (6 o'clock)
    d.line([(cx - rim_r, cy), (cx + rim_r, cy)], fill=copper, width=round(spoke_w))
    d.line([(cx, cy), (cx, cy + rim_r)], fill=copper, width=round(spoke_w))
    for px, py in [(cx - rim_r, cy), (cx + rim_r, cy), (cx, cy + rim_r)]:
        d.ellipse([px - cap, py - cap, px + cap, py + cap], fill=copper)
    # hub
...
def main() -> None:
    ...
    print("wrote icon.png, adaptive-icon.png, splash-icon.png to", os.path.normpath(OUT))
